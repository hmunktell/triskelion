...
module carriage() {
	difference() {
		union() {
			...
			translate([0,-linear_bearing_dia/2,0]) cube([smooth_rod_distance+linear_bearing_dia,10,linear_bearing_height], center=true);
			...
			for(i=[-smooth_rod_distance/2,smooth_rod_distance/2]) {
				translate([i,0,0]) {
					...
					translate([0,7.5,0]) cube([linear_bearing_dia,15,linear_bearing_height], center=true);
				}
			}
		}
